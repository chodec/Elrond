...
def create_new_exercise_plan(
    exercise_plan_data: ExercisePlanCreate, db: Session = Depends(get_db)
):
    try:
...
def get_exercise_plan_by_id(exercise_plan_id: UUID, db: Session = Depends(get_db)):
    db_plan = read_exercise_plan(
        db=db, trainer_id=get_current_trainer_id(), exercise_plan_id=exercise_plan_id
    )
...
def get_all_exercise_plans(search: Optional[str] = None, db: Session = Depends(get_db)):
...
def delete_existing_exercise_plan(
    exercise_plan_id: UUID, db: Session = Depends(get_db)
):
