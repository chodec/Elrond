...
def get_my_meal_plans(
    client_id: UUID = Depends(get_current_client_id), db: Session = Depends(get_db)
):
    db_plans = read_client_meal_assignments(db=db, client_id=client_id)
...
def get_my_exercise_plans(
    ...
    db_plans = read_client_exercise_assignments(db=db, client_id=client_id)
