...
from app.schemas.client_measurement import ClientMeasurementCreate, ClientMeasurementUpdate

def read_measurement_by_id(db: Session, measurement_id: UUID) -> Optional[ClientMeasurement]:
    return db.query(ClientMeasurement).filter(
...
def read_measurements_by_client_id(
    ...
    return db.query(ClientMeasurement).filter(
        ClientMeasurement.client_id == client_id
    ).order_by(ClientMeasurement.date.desc()).offset(skip).limit(limit).all()
