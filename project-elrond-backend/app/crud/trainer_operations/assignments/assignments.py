...
from app.crud.trainer_operations.subscriptions.client_subscription_validation import is_client_subscription_active 
...
def create_meal_plan_assignment(
    ...
    if not is_client_subscription_active(db, client_id, trainer_id):
        raise AssignmentError("Client does not have an active and valid subscription with this trainer.")
    
    db_assignment = MealPlanAssignment(
        client_id=client_id,
...
def read_client_meal_assignments(
    db: Session,
    client_id: UUID
) -> List[MealPlan]:
...
def create_exercise_plan_assignment(
    ...
    if not is_client_subscription_active(db, client_id, trainer_id):
        raise AssignmentError("Client does not have an active and valid subscription with this trainer for exercise plan assignment.")
...
def read_client_exercise_assignments(
    ...
    statement = (
        select(ExercisePlan)
        .join(ExercisePlanAssignment, ExercisePlan.id == ExercisePlanAssignment.exercise_plan_id)
